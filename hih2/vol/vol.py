...
def gd14(nh, met, uv, scale, dens_unit = u.cm**-3, scale_unit = u.pc):
	"""
	...
	Parameters: 
		nh (arr-like): neutral hydrogen number density (cm^-3 by default)
		met (arr-like): metallicity in solar units
		scale (arr-like): scale of grid cell (pc by default)
		uv (arr-like): normalized UV field strength
		dens_unit (astropy units object): units attached to input nh
	...
	g = (dmw**2 + dstar**2)**(1/2)
	lam = np.log(1 + (0.05/g + uv)**(2/3) * g**(1/3)/ustar)
	nhalf = nstar*lam/g
	w = 0.8 + lam**(1/2)/s**(1/3)
	x = w*np.log(nh/nhalf)
...
def kmt09b(nh, met, scale, uv = None, dens_unit = u.cm**-3, scale_unit = u.pc):
	"""
...
def k13(nh, met, uv, scale, rho_sd = 1e-2, fc = 1, iter_ = True, niter = 10, 
		dens_unit = u.cm**-3, scale_unit = u.pc, sddens_unit = u.Msun/u.pc**3):
	"""
...
def s14(nh, met, uv, scale, fc = 1, dens_unit = u.cm**-3, scale_unit = u.pc):
	"""
